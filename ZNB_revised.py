...
class ZNBChannel(InstrumentChannel):
    ...
    def __init__(self, parent: 'ZNB', name: str, channel: int,
        ...
        self.add_parameter(name='ftt_freq',
                           label='Two tone readout frequency',
                           get_cmd=f'SOUR{n}:FREQ1:CONV:ARB:IFR?',
                           set_cmd=self._set_ftt_freq,
                           get_parser=float,
                           vals=vals.Numbers(self._parent._min_freq,
                                             self._parent._max_freq - 10))
        self.add_parameter(name='ftt_power',
                           label='Two tone readout power',
                           get_cmd=f'SOUR{n}:POW1:OFFS?',
                           set_cmd=self._set_ftt_power,
                           get_parser=float,
        ...
        self.add_function('autoscale',
                          call_cmd='DISPlay:TRACe1:Y:SCALe:AUTO ONCE, '
                                   f"{self._tracename}")
        self.add_function('ftt_on', call_cmd=f'SOUR{n}:FREQ:CONV:ARB:EFR1 ON, 1, 1, 0, SWE')

        self.add_function('ftt_off', call_cmd=f'SENS{n}:FREQ:CONV FUND')
    ...
    @staticmethod
    def _set_ftt_freq(self, val: float):
        channel = self._instrument_channel
        self.write(f'SOUR{channel}:FREQ1:CONV:ARB:IFR 0, 1, {val:.7f}, FIX')
        self.write(f'SOUR{channel}:FREQ2:CONV:ARB:IFR 0, 1, {val:.7f}, FIX')

    def _set_ftt_power(self, val: float):
        channel = self._instrument_channel
        self.write(f'SOUR{channel}:POW1:OFFS {val:.4f}, ONLY')
        self.write(f'SOUR{channel}:POW2:OFFS {val:.4f}, ONLY')
